<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="客户分类" prop="base_category_name" width="180px" align="center"></el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span style="color:red;">{{ scope.row.create_time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="下次回访时间"
      ...
      >
        <template slot-scope="{row}">
          <span
            class="link-type"
          >{{ row.revisit_next_time | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="预约时间"
      ...
      >
        <template slot-scope="{row}">
          <el-tag>
            {{ row.order_time | parseTime('{y}-{m}-{d} {h}:{i}') }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="{row}">
          ...
          >
            {{ row.arrive_time | parseTime('{y}-{m}-{d}') }}
          </el-button>
          <!-- <el-button
            size="mini"
            type="success"
...
export default class extends Vue {
  ...
  private async getList() {
    ...
    const { data } = await getSickList(this.listQuery)
    console.log(data)
    this.list = data.data
